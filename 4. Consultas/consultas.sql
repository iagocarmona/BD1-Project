...
-- 1. Selecione o nome dos clientes que utilizaram um computador com sistema 'Linux Ubuntu 64Bits'.
SELECT C.nome FROM CLIENTE C, CLIENTE_UTILIZA_EQUIPAMENTO CE, EQUIP_COMPUTADOR EC
WHERE C.cpf = CE.cpfCliente
AND CE.idEquip = EC.idEquip
AND EC.sistema = 'Linux Ubuntu 64Bits';

-- 2. Selecione o nome dos clientes que não utilizaram nenhum equipamento.
SELECT C.nome FROM CLIENTE C
WHERE C.nome NOT IN (
		SELECT C.nome FROM CLIENTE C1, CLIENTE_UTILIZA_EQUIPAMENTO CE
        WHERE C.cpf = CE.cpfCliente
);

-- 3. Selecione o nome do fornecedor e o nome da empresa que forneceu peça de computador do modelo 'GTX 1050 OC 4GB'.
SELECT F.nomeFornecedor, F.nomeEmpresa FROM FORNECEDOR F, FORNECEDOR_FORNECE_PRODUTO FP, PRODUTO_PECASCOMPUTADOR PC
WHERE F.cnpj = FP.cnpjFornecedor
AND FP.idProduto = PC.idProduto
AND PC.modelo = 'GTX 1050 OC 4GB';

-- 4. Selecione o nome dos clientes (sem repetição) fizeram reparos cujo o tipo do equipamento é 'notebook' ou 'desktop'.
SELECT DISTINCT C.nome FROM CLIENTE C, REPARO R
WHERE C.cpf = R.cpfCliente
AND R.tipoEquipamento = 'notebook'
OR R.tipoEquipamento = 'desktop';

-- 5. Selecione o nome dos jogos que foram produzidos pela 'DICE' ou pela 'Valve' ou está instalado em um computador cujo sistema é 'Windows 11 64Bits'.
SELECT J.nome FROM JOGO J
WHERE J.nomeProdutora = 'DICE'
OR J.nomeProdutora = 'Valve'
UNION
SELECT J1.nome FROM JOGO J1, EQUIPAMENTO_TEM_JOGO EJ, EQUIP_COMPUTADOR EC
WHERE J1.id = EJ.idJogo
AND EC.idEquip = EJ.idEquip
AND EC.sistema = 'Windows 11 64Bits';

-- 6. Faça uma consulta em que forneça o id do equipamento que possui pelo menos 2 jogos instalados.
SELECT E.id FROM EQUIPAMENTO E, EQUIPAMENTO_TEM_JOGO EJ
WHERE E.id = EJ.idEquip
GROUP BY EJ.idEquip
HAVING EJ.idEquip >= 2;

-- 7. Selecione o nome do funcionário que atendeu o cliente cujo nome contenha 'Stef'
SELECT F.nome FROM FUNCIONARIO F, FUNCIONARIO_ATENDE_CLIENTE FC, CLIENTE C
WHERE F.cpf = FC.cpfFuncionario
AND C.cpf = FC.cpfCliente
AND C.nome LIKE '%Stef%';

-- 8. Forneça o nome do funcionário que receba um salário menor que 2000 e não atendeu nenhum cliente.
SELECT F.nome FROM FUNCIONARIO F
WHERE F.salario < 2000
AND F.nome NOT IN (
		SELECT F1.nome FROM FUNCIONARIO F1, FUNCIONARIO_ATENDE_CLIENTE FC
        WHERE FC.cpfFuncionario = F1.cpf
);

-- 9. Selecione o nome dos clientes que utilizaram um equipamento e foi atendido pelo funcionario cujo nome é 'Dexter Akitt'.
SELECT C.nome FROM CLIENTE C, CLIENTE_UTILIZA_EQUIPAMENTO CE, FUNCIONARIO F, FUNCIONARIO_ATENDE_CLIENTE FC
WHERE C.cpf = CE.cpfCliente
AND FC.cpfCliente = C.cpf
AND FC.cpfFuncionario = F.cpf
AND F.nome = 'Dexter Akitt';

-- 10. Selecione o nome dos clientes que utilizou um equipamento que tenha o jogo cujo nome é 'BattleField 2' 
-- e foi instalado em um computador cujo sistema é 'Windows 11 64Bits'.
SELECT C.nome FROM CLIENTE C, CLIENTE_UTILIZA_EQUIPAMENTO CE, EQUIPAMENTO_TEM_JOGO EJ, JOGO J
WHERE C.cpf = CE.cpfCliente
AND EJ.idEquip = CE.idEquip
AND J.id = EJ.idJogo
AND J.nome = 'BattleField 2'
AND J.nome IN (
		SELECT J1.nome FROM JOGO J1, EQUIPAMENTO_TEM_JOGO EJ, EQUIP_COMPUTADOR EC
        WHERE EJ.idJogo = J.id
        AND EJ.idEquip = EC.idEquip
        AND EC.sistema = 'Windows 11 64Bits'
);
